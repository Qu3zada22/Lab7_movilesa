package com.example.lab7_moviles.ui.meals.view
...
import androidx.compose.material3.Text  // Usando Material3 para Text
...
import com.example.lab7_moviles.networking.response.Meals
import com.example.lab7_moviles.ui.mealdetail.view.MealDetail
import com.example.lab7_moviles.ui.meals.viewmodel.MealFilterViewModel
...
fun MealFilterMainApp(category: String?) {
    ...
    val filteredMeals: MutableState<List<Meals>> = remember { mutableStateOf(emptyList()) } // Lista de estado mutable de las comidas que pertenecen a la categoría seleccionada
    val context = LocalContext.current // Contexto local

    if (category != null) { // Llama a la función para obtener las comidas de cada categoría desde la API
...
fun MealItem(meal: Meals, context: Context) {
    val backgroundColor = Color(225, 215, 245)
    Box(
        modifier = Modifier
            .fillMaxWidth()
            .padding(8.dp)
